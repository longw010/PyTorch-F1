...
def decodernw_onetime(
    ...
    i = len(num_channels_up) - 1
    if upsample_first:
        model.add(conv(num_channels_up[0], num_channels_up[1], 2**filter_size_up, 1, pad=pad))
        ...
        if upsample_mode != 'none' and i != 0:
            model.add(nn.Upsample(scale_factor=2, mode=upsample_mode))
        # model.add(nn.functional.interpolate(size=None,scale_factor=2, mode=upsample_mode))
        model.add(conv(num_channels_up[0], num_channels_up[1], filter_size_up[i], 1, pad=pad))
